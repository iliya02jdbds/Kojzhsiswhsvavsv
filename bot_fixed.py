...
REQUIRED_CHANNEL_USERNAME = "lllliiiiooooiiiiillll"       # بدون @ و بدون لینک
...
async def cfg_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🌐 اول تلاش می‌کنیم خودکار از پنل بسازیم؛ اگه هر دلیلی شکست خورد،
    # سفارش pending می‌مونه و می‌ره رو همون مسیر قدیمی (ارسال دستی توسط ادمین)
    panel_result = None
    panel_error = None
    try:
        panel_result = nahan_add_user(
            username=f"user{uid}",
            volume_gb=volume,
            days=DEFAULT_PANEL_EXPIRY_DAYS,
        )
    except NahanError as e:
        panel_error = str(e)
        logger.warning("panel auto-delivery failed for order #%s: %s", order_id, panel_error)
    ...
    if panel_result:
        ...
        await safe_edit(
            ...
            f"📦 سفارش #{order_id} — {fmt_volume(volume)} گیگابایت — {DEFAULT_PANEL_EXPIRY_DAYS} روز\n\n"
            f"🔗 لینک اشتراک شما:\n`{sub_url}`\n\n"
            "این لینک رو تو اپلیکیشن VPN (مثل v2rayNG) وارد کن.\n"
    ...
    if get_setting("purchase_notify") == "1":
        try:
            ...
            fail_note = f"\n⚠️ تحویل خودکار شکست خورد: {md_escape(panel_error)}" if panel_error else "\n⚠️ NAHAN_API_KEY تنظیم نشده"
            await notify_admins(
                context,
                f"🛍 *خرید جدید*\n━━━━━━━━━━━━━━\n"
                f"👤 {md_escape(user['first_name'] or 'ناشناس')} (`{uid}`)\n"
