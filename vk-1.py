...
async def kos_receive_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["kos_text"] = update.message.text
    kb = InlineKeyboardMarkup([
...
async def kos_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    text = context.user_data.pop("kos_text", "")
    if not text:
    ...
    for r in rows:
        try:
            # بدون هیچ پیشوند یا فرمت اضافه؛ دقیقاً همون متنی که فرستاده شد
            await context.bot.send_message(r["id"], text)
            sent += 1
        except Exception:
            failed += 1
        await asyncio.sleep(BROADCAST_DELAY)
...
def main():
    ...
    kos_conv = ConversationHandler(
        ...
        states={
            KOS_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, kos_receive_text)],
